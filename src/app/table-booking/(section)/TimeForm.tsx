...
const TimeForm: FC<TimeFormProps> = ({ mainform, setpage }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-3">
      <p className="text-3xl font-semibold text-[#F7F4ED]">
        ...
        >
          <div className="grid w-full max-w-[650px] grid-cols-1 gap-6 py-12 md:grid-cols-2">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-[#F7F4ED]">No of Guests</FormLabel>
                  <Select
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="">
                  <FormLabel className="text-[#F7F4ED]">Date</FormLabel>
                  <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          variant={"outline"}
                          className={`h-12 w-full justify-start border-muted bg-transparent text-left font-normal text-[#F7F4ED] hover:text-[#F7F4ED]`}
                        >
                          <CalendarIcon className="mr-2 h-4 w-4 text-[#F7F4ED]" />
                          {field.value ? (
                            format(field.value, "PPP")
                          ) : (
                            <span className="text-[#F7F4ED]">Pick a date</span>
                          )}
                        </Button>
                      </FormControl>
                    </PopoverTrigger>
            ...
            {form.watch("date") ? (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-[#F7F4ED]">Time</FormLabel>
                    <Select
                      onValueChange={field.onChange}
                      defaultValue={field.value}
                    >
          ...
          {form.watch("date") && (
            <div className="flex w-full flex-col gap-4 py-12">
              <p className="text-[#F7F4ED]">Choose an available time slot:</p>
              <div className="custom-scrollbar grid h-[250px] grid-cols-2 gap-6 overflow-y-scroll md:grid-cols-4 lg:grid-cols-5">
                {generateTimeSlots().map((time) => (
                  <div
                    ...
                    className={cn(
                      "flex h-full w-full cursor-pointer items-center justify-center border-[1px] border-gray-800 py-4 text-[#F7F4ED]",
          ...
          <Button
            className="w-fit bg-[#F7F4ED] px-6 py-6 text-[#317299]"
            disabled={disable}
          >
            Book A Table
          </Button>
